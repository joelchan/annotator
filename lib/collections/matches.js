...
DocMatch = function(seedDoc, matchDoc, user, searchType, search) {
    ...
    return {
        possibleMatch: function(seedDoc, matchDoc, query, rank) {
            ...
            if (oldMatch) {
                ...
                EventLogger.logPossibleMatch(matchDoc, "best", oldMatch);
            } else {
                ...
                var match = new DocMatch(seedDoc,
                                         matchDoc,
                                         user,
                ...
                EventLogger.logPossibleMatch(matchDoc, "notMatch", match);
                return match;
            }
        },
        notMatch: function(seedDoc, matchDoc, query, rank) {
            ...
            if (toRemove) {
                ...
                EventLogger.logRejectMatch(matchDoc, previousState, toRemove);
        ...
        bestMatch: function(seedDoc, matchDoc, query, rank) {
            ...
            if (oldMatch) {
                ...
                EventLogger.logBestMatch(matchDoc, "possible", oldMatch);
                return oldMatch;
                // return true;
            } else {
                ...
                match.bestMatch = true;
                match._id = DocMatches.insert(match);
                // Documents.update({_id: seedDoc._id},{$addToSet: {matchIDs: newMatch._id}});
                EventLogger.logBestMatch(matchDoc, "notMatch", match);
                return match;
                // return false;
            }
        },
        updateMatches: function(user, currentDoc, searchType) {
          ...
          var lastMatchSet = Session.get("lastSearch").matches.slice(0,marker);
          var newMatchSet = [];
          var ranks = {};
          var rank = 1;
