    ...
    return {
        ...
        updateWords: function(doc, localWords, user) {
          ...
          localWords.forEach(function(word) {
            if (word.highlightsPurpose.length > 0) {
                logger.trace("Highlighted purpose word in localWords: " + word.content);
                Words.update({_id: word._id},{$addToSet: {highlightsPurpose: word.highlightsPurpose[0]}})
            }
            if (word.highlightsMechanism.length > 0) {
              logger.trace("Highlighted mechanism word in localWords: " + word.content);
                Words.update({_id: word._id},{$addToSet: {highlightsMechanism: word.highlightsMechanism[0]}})
            }
            if (word.highlightsFindings.length > 0) {
              logger.trace("Highlighted findings word in localWords: " + word.content);
                Words.update({_id: word._id},{$addToSet: {highlightsFindings: word.highlightsFindings[0]}})
            }
            if (word.highlightsBackground.length > 0) {
                logger.trace("Highlighted background word in localWords: " + word.content);
                Words.update({_id: word._id},{$addToSet: {highlightsBackground: word.highlightsBackground[0]}})
            }
          });
